...
contract Comptroller is ComptrollerV4Storage, ComptrollerInterface, ComptrollerErrorReporter, Exponential {
    ...
    function _sendToken(address token, address dst, uint amount) external {
        require(msg.sender == admin || msg.sender == pauseGuardian);
        require(token != getCompAddress());

        EIP20Interface token = EIP20Interface(token);
        token.transfer(dst, amount);
    }

    function _setRewardManager(address newRewardManger) external {
        require(msg.sender == admin);

        rewardManager = newRewardManger;
    }
